...
public class MySQLDataExtractor implements DataExtractor {
    ...
    @Override
    public String getName() { return "MySQLDataExtractor"; }

    @Override
    public String getFullName() { return "com.migration.extractor.MySQLDataExtractor"; }

    @Override
    public String getDescription() { return "MySQL数据库数据提取器"; }

    @Override
    public FlowEngine.NodeResult execute(FlowEngine.FlowContext context, FlowNode node) {
        log.info("MySQL数据提取开始: nodeId={}", node.getNodeId());
        ...
        log.info("连接MySQL: {}, 查询: {}", jdbcUrl, sql);
        ...
        try (Connection conn = DriverManager.getConnection(jdbcUrl, username, password)) {
            ...
            log.error("MySQL查询失败: {}", e.getMessage(), e);
            return FlowEngine.NodeResult.fail("MySQL查询失败: " + e.getMessage());
        }
